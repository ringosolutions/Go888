...
cc.Class({
	...
	onLoad () {
		...
		this.onCuocClick = function(){
			...
			if (isNaN(bet) || bet < 1000) {
				...
				noticeComponent.text.string = 'Tiền cược phải lớn hơn 1.000';
	...
	onData: function(data){
		...
		if(void 0 !== data.logs){
			...
			this.dataLogs();
			this.setPhien();
			if(cc.RedT.setting.taixiu.time_remain > 40){//60
				this.setDice(!0);
		...
		if(void 0 !== data.finish){
			if (cc.RedT.setting.taixiu.getLogs) {
				...
				if(this.isNan){
					...
					this.diaNan.position       = cc.v2(0,+2);
			...
			cc.RedT.setting.taixiu.time_remain = 52;
	...
	playTime: function(){
		...
		this.timeInterval = setInterval(function() {
			if (this.isNan){
					if ((cc.RedT.setting.taixiu.time_remain > 0 ) && (cc.RedT.setting.taixiu.time_remain < 2)){
			...
			if (cc.RedT.setting.taixiu.time_remain > 41) {//61
				if (this.bg_Dice._animator !== null && this.bg_Dice._animator.isPlaying) {
					this.bg_Dice.stop();
				}
				this.bg_efDice.active = false;
				var time = helper.numberPad(cc.RedT.setting.taixiu.time_remain-42, 2);//62
				...
				if (cc.RedT.setting.taixiu.time_remain < 45) {//71 51
					this.efStop();
				}
				if (cc.RedT.setting.taixiu.time_remain < 45) {//66 46 
	...
	nextRealTime: function(){
		if (cc.RedT.setting.taixiu.time_remain > 41) {//61
			...
			this.nodeTimeWait.active  = true;
			this.timeCuoc.node.active = false;
			var time = helper.numberPad(cc.RedT.setting.taixiu.time_remain-42, 2);//62 42
	...
	onDataTaiXiu: function(data){
		void 0 !== data.red_tai        && (Tween.numberTo(this.totalLeft, data.red_tai, 0.3));
		void 0 !== data.red_tai        && (Tween.numberTo(this.totalRight, data.red_xiu, 0.3));
